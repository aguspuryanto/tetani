...
class Appsettings_model extends CI_model
{
    ...
    public function ubahdataappsettings($data)
    {
        ...
        $this->db->set('jasaapp', $data['jasaapp']);
        $this->db->where('id', '1');
    ...
    public function ubahwhatsapp($data)
    {
        $this->db->set('whatsappserver', $data['whatsappserver']);
        $this->db->set('whatsappapi', $data['whatsappapi']);
        $this->db->set('whatsappnumber', $data['whatsappnumber']);
        $this->db->where('id', '1');
        $this->db->update('app_settings', $data);
    }
}
