...
class wallet_model extends CI_model
{
    ...
    public function send_notif($title, $message, $topic)
    {
        ...
        $headers = array(
            ...
            'Authorization: key=' . $this->config->item("app_name")
        );
        $curl = curl_init();
    ...
    public function updatesaldowalletwithdraw($data, $data2)
    {
        ...
        if ($saldobaru < 0) {
            ...
            $numberreff = rand ( 10000 , 99999 );
            $this->db->set('status', '1');
            $this->db->set('reff', 'withdraw'.$numberreff);
    ...
    public function updatekadaluarsa()
    {
        ...
        foreach ($wallet as $wl) {
            ...
            if ($tanggal <= $tanggal2 && $wl['status'] == 0 && $wl['expired'] != "") {
                ...
                $this->Notification_model->send_notif_topup($serverkey,$title, $wl['id_user'], $desc, $wl['invoice'], $token);
            }
        }
    }
}
